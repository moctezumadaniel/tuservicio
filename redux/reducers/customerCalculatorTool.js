...
export const customerCalculatorToolReducer = (state = initialCalculatorState, action) => {
    switch (action.type) {
        case 'WRITE_NUMBER_OR_SIGN_IN_CALCULATOR_TOOL_OPERATION':
            ...
            else if (/\W(00+)$/g.test(state + action.numberOrSign) ||
                /^(0+)/g.test(state + action.numberOrSign) ||
                /\W([.]+)$/g.test(state + action.numberOrSign) ||
                /^([.]+)/g.test(state + action.numberOrSign) ||
                /(\d+[.]\d+[.])/g.test(state + action.numberOrSign)
            ) {
                return state
            }
            else {
