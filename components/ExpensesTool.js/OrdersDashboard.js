...
import {
  changeExpenseToolToEditingExpense,
  changeExpenseToolToExpense,
...
function ExpensesList() {
  ...
  function editForm(expense) {
    dispatch(
      loadExpensesToolEditingFormDate(expense),
      dispatch(changeExpenseToolToEditingExpense())
    );
  }
  function deleteExpense(customerId, id) {
    axios
      .delete(`api/customer/deleteCustomerExpense`, {
        params: {
          customerId,
          id,
        },
        ...
        {expenses.map((expense) => (
          <div className={styles.ExpenseItemContainer} key={expense._id}>
            <div className={styles.ExpenseMainInformation}>
              <div>{expense.date ? expense.date.slice(0, 10) : ""}</div>
              <div>{`$${expense.amounth}`}</div>
            </div>
            <div className={styles.ExpenseDescription}>
              {expense.description.slice(0, 141)}
            </div>

            <div className={styles.ExpenseButtonsContainer}>
              <button
                className={styles.OpenEditItemButton}
              ...
              <button
                ...
                onClick={() =>
                  deleteExpense(customerInformation.customerId, expense._id)
                }
              >
                {deleteButton}
              </button>
...
function OrdersDashboard() {
  ...
  const dateRangeTotal = () => {
    const total = expenses.reduce(
      (acumulator, expense) => acumulator + (expense.amounth || 0),
      0
    );
    return total;
  };
  ...
  return (
    <div className={styles.ExpensesToolMainContainer}>
      <div className={styles.RegisterExpenseContainer}>
        <button
          className={styles.RegisterExpenseButton}
          onClick={handleNewExpensePress}
        >
          {registerExpenseButton}
        </button>
      </div>
      <button className={styles.CustomDateRangeButton}>
        {customExpensesRange}
      </button>
      <div className={styles.DateRangeSelected}>{dateRangeSelected}</div>

      <div className={styles.GrandTotalContainer}>
        <div className={styles.GrandTotalDescription}>
          {dateRangeTotalDescription}
        </div>
