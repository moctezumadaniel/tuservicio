...
import {
  addItemToCustomerEditingOrderForm,
...
function ListOfItems() {
  ...
  const handleChangeOrderItemDescription = (id) => (event) => {
    dispatch(
      changeCustomerEditingOrderFormItemDescription(id, event.target.value)
    );
  };
  return (
    <div>
        ...
        return (
          <div key={item.id}>
            <div className={styles.OrderItemContainer}>
              <textarea
                className={styles.OrderItemDescrription}
...
function Order() {
  ...
  const orderDate = useSelector((state) =>
    state.customerEditingOrder.date.slice(0, 10)
  );
  const orderDescription = useSelector(
    (state) => state.customerEditingOrder.description
  );
  const newItemDescription = useSelector(
    (state) => state.customerEditingOrder.newItemDescription
  );
  const newItemAmounth = useSelector(
    (state) => state.customerEditingOrder.newItemAmounth
  );
  ...
  function getOrderNumber(orders, date) {
    ...
    const numbersFromOrderWithSelectedDate = orders.map((order) => {
  ...
  const handleDateChange = (event) => {
    dispatch(
      changeCustomerEditingOrderFormDate(event.target.value),
      dispatch(
        changeCustomerEditingOrderFormNumber(
          getOrderNumber(orders, event.target.value)
        )
      )
    );
  };
  ...
  const handleNewItemDescriptionChange = (event) => {
    dispatch(
      changeCustomerEditingOrderFormNewItemDescription(event.target.value)
    );
  };
  ...
  function saveEditingOrder(customerId, editingOrder) {
    ...
    axios
      ...
      .then((response) => {
        if (response.data) {
          dispatch(
            updateCustomerInformationOrders(response.data.orders),
  ...
  return (
    <div className={styles.OrderFormMainContainer}>
      ...
      <div
        className={styles.OrderFormTitle}
      >{`${orderNumber}-${orderDate} ${editingTitle}`}</div>
      ...
      <div className={styles.OrderDateDescriptionContainer}>
        ...
        <input
          type="text"
          className={styles.OrderDescriptionInput}
      ...
      <div className={styles.OrderItemForm}>
        ...
        <textarea
          className={styles.DescriptionOrderInput}
        ...
        <input
          type="number"
          className={styles.AmounthOrderInput}
        ...
        {newItemDescription && newItemAmounth ? (
          <button
            className={styles.SaveNewItemButton}
