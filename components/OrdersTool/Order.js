...
function Order() {
  ...
  const newItemDescription = useSelector(
    (state) => state.customerOrderToolForm.newItemDescription
  );
  const newItemAmounth = useSelector(
    (state) => state.customerOrderToolForm.newItemAmounth
  );
  ...
  const itemDescription =
    "Descripcion del articulo, servicio o cualquier cosa para lo que el cliente utilice las ordenes";
  ...
  function addCustomerOrder(customerId, newOrder) {
    if (!newOrder.date || !newOrder.number) return;
    axios
      .post(`api/customer/addCustomerOrder`, {
        customerId,
