...
import {
  changeCustomerReservationsFormToAddReservation,
  changeCustomerReservationsFormToEditReservation,
} from "../../../redux/actions/CustomerReservationsForms";
...
function ListOfReservations() {
  const listOfReservations = useSelector(
    (state) => state.customerPublicInformation.listOfReservations
  );
  const customerId = useSelector(
    (state) => state.customerPublicInformation.customerId
  );
  const dispatch = useDispatch();
  const configureButton = "EDITAR";
  const doneButton = "ATENDIDA";
  console.log(
    useSelector((state) => state.customerPublicInformation.listOfReservations)
  );

  function openEditReservationForm(reservation) {
    dispatch(
      loadCustomerEditingReservation(reservation),
  ...
  function deleteReservation(customerId, id) {
    axios
      .delete(`api/customer/deleteCustomerReservation`, {
        params: {
          customerId,
          id,
        },
      ...
      .then((response) => {
        if (response.data) {
          dispatch(
            updateCustomerPublicInformationListOfReservations(
              response.data.listOfReservations
            )
          );
      ...
      {listOfReservations.map((reservation) => (
        <div className={styles.ReservationItem} key={reservation._id}>
          <div className={styles.ItemSchedule}>
            {`${reservation.date || ""} de ${reservation.start || ""} a ${
              reservation.end || ""
            }`}
          </div>
          ...
          <div className={styles.ReservationItemButtonsContainer}>
            ...
            <button
              className={styles.AttendedReservationButton}
    ...
    <div className={styles.MainContainer}>
      ...
      <div className={styles.AddReservationContainer}>
        <button
          ...
          onClick={handleAddReservationPress}
        >
          {addReservation}
        </button>
      </div>
