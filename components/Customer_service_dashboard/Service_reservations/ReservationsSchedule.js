...
import {
  changeCustomerReservationsFormToAddSchedule,
  changeCustomerReservationsFormToEditSchedule,
...
function ScheduleList() {
  ...
  const customerSchedules = useSelector(
    (state) => state.customerPublicInformation.listOfSchedules
  );
  ...
  return (
    <div className={styles.ScheduleListContainer}>
      {customerSchedules && currentPeriod === "Dias laborales"
        ? customerSchedules.map((schedule) => (
            <div className={styles.ScheduleItem} key={schedule._id}>
              ...
              <div className={styles.ScheduleButtonsContainer}>
                <button
                  className={styles.EditItemSchedule}
        ...
        ? customerSchedules.map((schedule) => {
            if (
              schedule.day === currentPeriod ||
              schedule.day === "Dias laborales"
            ) {
              return (
                <div className={styles.ScheduleItem} key={schedule._id}>
                  <div
                    className={styles.ScheduleTimeContainer}
                  >{`${schedule.start} a ${schedule.end}`}</div>
                  <div className={styles.ScheduleDescription}>
                    {schedule.description}
                  </div>
  ...
  return (
    <div className={styles.MainContainer}>
      ...
      <div className={styles.DaysSelectionContainer}>
        <select
          className={styles.DaysSelection}
